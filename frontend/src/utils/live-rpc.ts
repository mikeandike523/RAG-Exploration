...
export class LiveRouteError<T extends SerializableObject | void  = void> extends Error {
 ...
 * @returns
 */
export default async function callLiveRoute<
...
>(
  ...
  try {
    const taskBeginResponse = await fetch(endpoint.replace(/\/+$/g, ""), {
      method: "POST",
      headers: {
    ...
    if (e instanceof LiveRouteError) {
      throw e;
    } else {
      throw new LiveRouteError(
